...
{
	return CastChecked<const UPFPathFollowerComponent>(Component);
	
}

UPFPathFollowerComponent* FPFPathFollowerVisulalizer::GetEditedPathFollower()
{
	return Cast<UPFPathFollowerComponent>(GetComponentFromPropertyName(FollowerOwningActor.Get(), FollowerCompPropName));
}
...
#endif
{
	...
	FollowerCompPropName = GetComponentPropertyName(FollowerComp);
	if (FollowerCompPropName.IsValid())
	{
		AActor* OldFollowerOwningActor = FollowerOwningActor.Get();
		FollowerOwningActor = FollowerComp->GetOwner();
		...
		FMenuBuilder MenuBuilder(true, /*global commands*/nullptr);
		{
			...
			MenuBuilder.BeginSection(NAME_None, FText::FromString("Actions"));
			{
				...
				TSharedRef<SWidget> DeleteButton = SNew(SButton)
					.OnClicked_Raw(this, &FPFPathFollowerVisulalizer::DeleteSpeedPoints)
	...
	if (_SelectedRotationPointIndex.Num() > 0)
	{
		FMenuBuilder MenuBuilder(true, /*global commands*/nullptr);
		{
			...
			MenuBuilder.BeginSection(NAME_None, FText::FromString("Actions"));
			{
				...
				TSharedRef<SWidget> DeleteButton = SNew(SButton)
					.OnClicked_Raw(this, &FPFPathFollowerVisulalizer::DeleteRotationPoint)
